...
ListOfUnique = [] # a list of unique station names
...
for n in inventoryP.index:                     # loop over all lines in the xls/ods sheet
    if (not math.isnan(inventoryP.iloc[n,3])): # the last loop is going over the last line
        ...
        if inventoryP.iloc[n,0] == 1 and inventoryP.iloc[n,11] >= 30 and inventoryP.iloc[n,11] < 40: # the 1st column (0th here) says, if it is IN the region (=1) or out (=0)
            ...
            if str(inventoryP.iloc[n,1]) + '.' + str(inventoryP.iloc[n,2]).partition("/")[0] not in ListOfUnique:
                ListOfUnique.append(str(inventoryP.iloc[n,1]) + '.' + str(inventoryP.iloc[n,2]).partition("/")[0])
                listALLb.write    ("%s\n" % (str(inventoryP.iloc[n,4]) + ' ' + str(inventoryP.iloc[n,3]) + ' ' + str(inventoryP.iloc[n,1]) + '.' + str(inventoryP.iloc[n,2]).partition("/")[0])) # list of all station coordinates
...
for n in inventoryT.index:                        # loop over all lines in the xls/ods sheet
    if inventoryT.iloc[n,1] != '5B': # if it is not 5B network, which is not part of AdA (yet)
        ...
        if str(inventoryT.iloc[n,1]) + '.' + str(inventoryT.iloc[n,2]).partition("/")[0] not in ListOfUnique:
            ListOfUnique.append(str(inventoryT.iloc[n,1]) + '.' + str(inventoryT.iloc[n,2]).partition("/")[0])
            listALLb.write    ("%s\n" % (str(inventoryT.iloc[n,4]) + ' ' + str(inventoryT.iloc[n,3]) + ' ' + str(inventoryT.iloc[n,1]) + '.' + str(inventoryT.iloc[n,2]).partition("/")[0])) # list of all station coordinates
